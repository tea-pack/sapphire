	...
	public void viewTest() {
		...
		try {
			...
			while ((values = csvParser.next()) != null) {
				...
				//System.out.printf("BC: %s -- %s : %ss%n", view.broadcast.start.format(ViewParser.DATE_TIME_FORMAT), view.broadcast.end.format(ViewParser.DATE_TIME_FORMAT), view.broadcast.duration.toSeconds());
				//System.out.printf("CL: %s -- %s : %ss%n%n", view.start.format(ViewParser.DATE_TIME_FORMAT), view.end.format(ViewParser.DATE_TIME_FORMAT), view.duration.toSeconds());
				//}
			}
			System.out.println(count);
		}
